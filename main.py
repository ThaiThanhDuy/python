    ...
    if retval:
        ...
        for i, points in enumerate(decoded_points):
            ...
            for j in range(len(points)):
                cv2.line(img, tuple(points[j]), tuple(
                    points[(j+1) % len(points)]), color=(255, 0, 0), thickness=2)
            # Hien thi du lieu ma QR tren hinh anh
            cv2.putText(img, data[i], (int(points[0][0]), int(
                points[0][1]) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            # In du lieu ma QR
